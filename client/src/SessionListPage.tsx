...
export default function SessionListPage(): JSX.Element {
    ...
    const projectId = createMemo(() => params.project_id as string);
    ...
    const columns = createMemo<ColumnDef<SessionMetadata>[]>(() => [
        {
            ...
            header: 'Session ID',
            cell: info => <A
                class="text-brand-700 hover:text-brand-600 hover:underline font-medium"
                href={`/sessions/${params.project_id}/${info.getValue()}`}>{String(info.getValue())}</A>,
            enableSorting: true,
